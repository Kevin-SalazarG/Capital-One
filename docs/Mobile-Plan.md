...
The liquidity revision now leads with unconditional capacity, then observed cash and the reserve target. It adds the returned scenario selector, an exact daily-closing chart, daily disclosure, a native provenance/assumptions sheet, and a read-only consultation action. The forecast visual separates the projected closing amount from the intraday minimum. Missing data, stale source state, conditional money, dates, and the evaluated horizon retain their documented meanings.

The final mobile run passed 92 tests in ten suites, including eight login, eight transition, ten chart, and nine liquidity integration tests. Source-policy passed for 144 first-party files and 35 virtual regression cases; encoding, mobile types/lint/formatting, Doctor 21/21, and iOS/Android exports passed. The iPhone 17 build with the native splash plugin succeeded, and subsequent UI changes ran through Metro. The complete backend/SDK workspace gate was not rerun for this UI revision.

The revised light overview/forecast, scenario switch, details sheet, native startup states, and restored authenticated session were observed in the simulator. A later desktop lock interrupted the remaining interactive review. Native keyboard/focus, VoiceOver, Dynamic Type, system appearance/reduced-motion changes, daily-disclosure interaction, and animation smoothness still need complete evidence. Jobs and decisions remain labeled placeholders. Final totals, candidate identity, and the native limitations are recorded in [mobile-login.md](verification/mobile-login.md). M1 and the native Android, physical iPhone, signing, and release gates remain open.
